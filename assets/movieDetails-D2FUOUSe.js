import"./styles-44WQKs1o.js";import{n as e}from"./MovieAPI-b16TYHji.js";import{n as t,r as n,t as r}from"./utils-BKqcSTl2.js";var i=`AIzaSyCJryAE3LLaJvwvBeYEPsYuENRimS6gpGc`,a=`https://www.googleapis.com/youtube/v3`;async function o(e){let t=await(await fetch(`${a}/search?part=snippet&q=${e} official trailer&type=video&key=${i}`)).json();return t.items?t.items.length>0?t.items[0]:null:(console.error(`Youtube API error:`,t),null)}var s=new URLSearchParams(window.location.search).get(`id`);async function c(){let i=await e(s),a=await o(i.title),c=i.genres.map(e=>e.name).join(`, `),l=Math.round(i.vote_average/2),u=`⭐`.repeat(l)+`✩`.repeat(5-l),d=document.querySelector(`#movie-details`),f=t().some(e=>e.id===i.id);d.innerHTML=`
